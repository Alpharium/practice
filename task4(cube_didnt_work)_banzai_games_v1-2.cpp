//interpolation-extrapolation (linear,square,cube) cube-didnt work
#include <stdio.h>
#include <iostream>
#include <vector>
#include <utility>
...
private: // Закрытая часть класса, её элементы доступны только внутри класса
	...
	vector<pair<double, double>> mas_of_roots;//exit
	ofstream fout;
	ifstream fin;
	int n_calc;
	int k_arg;
	...
	std::string file_name_out_roots;
	double y = 0.0;
public: // Открытая часть класса, её элементы доступны везде
	...
	vector<pair<double, double>> linear_computer(vector<pair<double, double>> mas_of_points_out, vector<double> mas_of_arguments_out) { // Через этот метод мы сможем получить значение закрытого поля
		...
		//std::cin >> file_name_out_roots;
		//file_name_out_roots = file_name_out_roots_input;
		//fout.open(file_name_out_roots);
		//fout << mas_of_arguments.size()<<"\n";
		for (int i = 0; i < k_arg;i++) {
			y = (a*mas_of_arguments[i] + b);
			mas_of_roots.push_back(std::make_pair(mas_of_arguments[i],y));

			//fout << mas_of_roots[i].first << " " << mas_of_roots[i].second<< "\n";
		}
		//fout.close();

		//vector<double> mas_of_linear_roots;
		return  mas_of_roots;     // Поле закрытое, но метод открытый
	};

	vector<pair<double, double>> square_computer(vector<pair<double, double>> mas_of_points_out, vector<double> mas_of_arguments_out) {
		...
		for (int i = 0;i < k_arg;i++) {
			y = a2*pow(mas_of_arguments[i], 2) + b2*mas_of_arguments[i] + c;
			mas_of_roots.push_back(std::make_pair(mas_of_arguments[i], y));
		}
		return mas_of_roots;

	}
	vector<pair<double, double>> cube_computer(vector<pair<double, double>> mas_of_points_out, vector<double> mas_of_arguments_out) {
		//-------------------------------------------------------- cube extrapolation-------------
		...
		for (int i = 0;i < n_calc;i++)
		{
			q += (pow(mas_of_points[i].first, 3)*mas_of_points[i].second);//Ex^3*y E-means"sum"
			w += (pow(mas_of_points[i].first, 2)*mas_of_points[i].second);//Ex^2*y
			e += mas_of_points[i].first*mas_of_points[i].second;//Ex^3*y
			r += mas_of_points[i].second;//Ey
			t += pow(mas_of_points[i].first, 6);//Ex^6
			u += pow(mas_of_points[i].first, 5);//Ex^5
			p += pow(mas_of_points[i].first, 4);//Ex^4
			g += pow(mas_of_points[i].first, 3);//Ex^3
			h += pow(mas_of_points[i].first, 2);//Ex^2
			m += mas_of_points[i].first;//Ex
		}
		...
		for (int i = 0;i < k_arg;i++) {
			y = a3*pow(mas_of_arguments[i], 2) + b3*pow(mas_of_arguments[i],2)+c3*mas_of_arguments[i] + d3;
			mas_of_roots.push_back(std::make_pair(mas_of_arguments[i], y));
		}
...
public:
	void set_file_name_out() {
		std::cout << "\n  Please enter the name of file for output\n";
		std::cin >> file_name_out;
	}
	std::string get_file_name_out() {
	...
	vector<pair<double, double>> get_base_points() {
		return mas_of_points;
	}
	...
	vector<double> get_argument_points() {
	...
	void writer_roots(vector<pair<double, double>> mas_of_roots_out) {
		fout.open(file_name_out);
		fout << mas_of_roots_out.size()<<"\n";
		for (int i = 0;i < mas_of_roots_out.size();i++){
			fout << mas_of_roots_out[i].first << " " << mas_of_roots_out[i].second<<"\n";
...
{
	int type;
	data_manager data_manager1;
	computer computer1;
	data_manager1.fftovpoints();
	data_manager1.fftovarguments();
	type = data_manager1.type_of_equation();
	data_manager1.set_file_name_out();
	
	switch (type) {
	case 1: {
		data_manager1.writer_roots( computer1.linear_computer(data_manager1.get_base_points(), data_manager1.get_argument_points()));
		break;
	}
	case 2: {
		data_manager1.writer_roots(computer1.square_computer(data_manager1.get_base_points(), data_manager1.get_argument_points()));
		break;
	}

	case 3: {
		data_manager1.writer_roots(computer1.cube_computer(data_manager1.get_base_points(), data_manager1.get_argument_points()));
