...
public: // Открытая часть класса, её элементы доступны везде
	//double real;
	vector<pair<double, double>> get_mas_of_points() { // Через этот метод мы сможем получить значение закрытого поля
		std::cin >> file_name;
		fin.open(file_name);
		fin >> n_calc;
		...
		return mas_of_points;     // Поле закрытое, но метод открытый
	};
};
